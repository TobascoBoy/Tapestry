...
struct StickerCanvasView: View {
    ...
    @State private var menuDragLocation: CGPoint = .zero
    @State private var hoveredMenuItem: PlusMenuItem? = nil
        ...
        NavigationStack {
            CanvasHostView(
                ...
                onDragChanged: { screenPos in
                    ...
                    let nearBottom = screenPos.y > screenHeight - 80
                    let inZone = screenPos.y > screenHeight - 50
                ...
                onDragEnded: { screenPos, stickerID in
                    let screenHeight = UIScreen.main.bounds.height
                    if screenPos.y > screenHeight - 50 {
    ...
    private var trashZone: some View {
        HStack(spacing: 8) {
            Image(systemName: trashIsTargeted ? "trash.fill" : "trash")
                .font(.system(size: trashIsTargeted ? 20 : 16, weight: .semibold))
        ...
        .background(
            Capsule()
                ...
                .overlay(
                    Capsule()
                        .stroke(trashIsTargeted ? Color.red.opacity(0.7) : Color.white.opacity(0.15), lineWidth: 1)
                )
        )
        .padding(.bottom, 20)
    }
        ...
        for sticker in sorted {
            ...
            case .music(let track):
                if musicViews[sticker.id] == nil {
                    ...
                    mc.onGestureStart = { [weak coordinator] in
                        coordinator?.canvasPan?.isEnabled = false
                        coordinator?.canvasPinch?.isEnabled = false
                    }
                    mc.onGestureEnd = { [weak coordinator] in
                        coordinator?.canvasPan?.isEnabled = true
                        coordinator?.canvasPinch?.isEnabled = true
                    }
            ...
            case .video(let videoURL, let thumbnail, let bgRemoved):
                if videoViews[sticker.id] == nil {
                    ...
                    vc.onGestureStart = { [weak coordinator] in
                        coordinator?.canvasPan?.isEnabled = false
                        coordinator?.canvasPinch?.isEnabled = false
                    }
                    vc.onGestureEnd = { [weak coordinator] in
                        coordinator?.canvasPan?.isEnabled = true
                        coordinator?.canvasPinch?.isEnabled = true
                    }
